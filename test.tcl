...
set prog_cmd [lindex $argv 0]
set basedir [lindex $argv 1]
set shell_prompt "$ "
set timeout 5
set test_fail 0
...
proc test_setup { testcount } {
	...
	puts "\n"
	puts "\u001b\[32m\[==========]\u001b\[0m Running $testcount tests."
...
# Start test fixture
#
proc testcase_setup { classname cmdline prompt } {
	# variable works like global. It allows access to global scope
	variable shell_prompt
	variable prog_cmd
	...
	set prog_prompt $prompt

	send -- "$prog_cmd $cmdline\r"
	# Disable echo for the send above
	log_user 0
...
# End test fixture
#
proc testcase_teardown { classname exitcmd } {
	# variable works like global. It allows access to global scope
	variable testcase_count
	variable test_start
	variable shell_prompt
...
proc testreport_xml { test_count test_fail test_report } {
	...
	puts $test_detail "  <testsuite name=\"$prog_cmd\" tests=\"$test_count\" failures=\"$test_fail\" disabled=\"0\" errors=\"0\" time=\"0\">"
...
set test_count [ llength $test_cases ]

test_setup $test_count
